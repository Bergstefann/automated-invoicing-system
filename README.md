...
This rebuild makes SQLite the source of truth. `lessons.billed_invoice_id` is a real, queryable column — a lesson is billed if and only if that column is set, full stop. The Sheet becomes a read source (`sync`) and a write-back target (status codes), not the place billing decisions get made. Everything that talks to Google — Sheets, Docs, Drive, Gmail — sits behind three Protocols (`SheetProvider`, `DocProvider`, `EmailProvider`) that the pipeline depends on exclusively; it never imports the concrete Google classes. Every test, and `--demo` mode, wires up in-memory fakes instead — that's what makes the whole thing runnable and testable with zero credentials and zero network access.
