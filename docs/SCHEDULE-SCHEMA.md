...
- **Text**, formatted `S-0001`, `S-0002`, … — stored and validated as a
  ...
  notation — a real failure mode for numeric-looking IDs in a spreadsheet.)
...
editing the workbook — what's safe to edit, a column glossary, one example
row — and isn't read by `invoicing.workbook` at all.
...
- **Silent skip on an unmatched name** — replaced by a required, specific
  ...
  unchanged — it's a live-Sheet-only heuristic, out of scope for this pass
...
resolves each Sheet-native first name to the database's `student_id` — the
one place in the whole pipeline where a name is still an identity lookup
